...
import "../GlobalVars.m" : __LIST, __SANITY_CHECK, __FRAME, __FLAT;
import "TensorSpc.m" : __GetTensorSpace;

// Similar to Magma's Flat, but handles how we need it.
__Flatten := function( X )
  if Type(X) in {SeqEnum, SetEnum, SetIndx, SetMulti, SetFormal} then
    if ISA(Type(Universe(X)), Rng) then
      // just one tensor given as a sequence of elements. 
      return [*X*]; 
    else
      // a sequence of stuff.
      return [*x : x in X*];
  ...
  if Type(X) in {List, Tup} then
    Y := [**];
    for x in X do
      if Type(x) in {SeqEnum, SetEnum, SetIndx, SetMulti, SetFormal, List, Tup} then
        Y cat:= $$(x);
      else
        Append(~Y, x);
      end if;
...
{Returns the associated tensor.}
  ...
  if T`Cat`Contra then
    Cat := TensorCategory( Arrows(T`Cat) cat [1], T`Cat`Repeats ); 
